...
for r=1:10
    ...
    [x,y,crv_cell,population_response]=normalize_SVM_decoder_5cross(0.1,0.05,temp_index,mode);
    all_results_5cross(r,:)=y;
    
    temp_index = find(cell2mat(cellfun(@isnan,all_spatial_info(:,2), 'UniformOutput', false))); %D1=NaN
    temp_index=temp_index(randperm(length(temp_index),200)); 
    [x,y,crv_cell,population_response]=normalize_SVM_decoder_10cross(0.1,0.05,temp_index,mode);
    all_results_10cross(r,:)=y;

end
save d0_5cross_100ms.mat all_results_5cross
save d0_10cross_100ms.mat all_results_10cross
%load d0_5cross_100ms.mat
%load d0_10cross_100ms.mat
%5 cross
accuracy_mat=mean(all_results_5cross);
x=linspace(-1.4,5,length(accuracy_mat));%57 
xq=linspace(-1.4,5,400);
...
save d1_5cross_100ms.mat all_results_5cross
save d1_10cross_100ms.mat all_results_10cross
%load d1_5cross_100ms.mat
%load d1_10cross_100ms.mat
...
%10 cross
accuracy_mat=mean(all_results_10cross);
x=linspace(-1.4,5.1,length(accuracy_mat));%57 
xq=linspace(-1.4,5.1,400);
