function [x_time,time_perform,crv_cell,population_response]=normalize_SVM_decoder_shuffle(win_size,step_size,temp_index, mode)
...
for i=1:length(edge1)
   % disp(i);
    ...
    shuffle_label=decode_label(randperm(length(decode_label)));
    OBJ=fitcecoc(population_response(:,:,i)',shuffle_label,'Coding','onevsall','Learners',t);
   ...
   % cvecoc = crossval(OBJ,'leaveout','on');
    ...
    confuse_mat=confusionmat(shuffle_label,Yhat);
