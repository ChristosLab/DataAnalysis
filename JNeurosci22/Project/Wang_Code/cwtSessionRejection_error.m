...
%%
global log_dir
log_dir = fullfile(project_dir, output_database, 'error_artifact_rejection_list.mat');
%%  Initiate values
if isfile(log_dir)
    load(log_dir)
else
...
cwt = squeeze(cwt_error_repo(ind_list(list_i, 1)).class(ind_list(list_i, 2)).cue_cwt);
plot_cwt(cwt)
%%  Reject sessions
k = 0;
