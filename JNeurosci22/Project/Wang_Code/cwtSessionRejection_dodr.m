zw_setpath
load(fullfile(project_dir, output_database, 'dodr_cwt_repo.mat'));
load(fullfile(project_dir, output_database, 'dodr_lfp_tbl.mat'));
%%
global list_i flag_list ind_list
%%
target_cwts = 1:numel(dodr_cwt_repo);
%%
ind_list = zeros(0, 2);
for i =  target_cwts
...
%%
global log_dir
log_dir = fullfile(project_dir, output_database, 'dodr_artifact_rejection_list.mat');
%%  Initiate values
if isfile(log_dir)
    load(log_dir)
else
...
c_forward.Callback = {@forwardButtonPushed, dodr_cwt_repo, c_flag};
c_backward = uicontrol('Position', [225, 25, 100, 50]);
c_backward.String = 'Previous trial';
c_backward.Callback = {@backwardButtonPushed, dodr_cwt_repo, c_flag};
f.KeyPressFcn = {@keyPress, c_backward.Callback, c_forward.Callback, c_flag};
%   Inititate first plot
cwt = squeeze(dodr_cwt_repo(ind_list(list_i, 1)).class(ind_list(list_i, 2)).cue_cwt);
plot_cwt(cwt)
%%  Reject sessions
k = 0;
session_flag_list = zeros(size(target_cwts));
for i = 1:numel(target_cwts)
    for j = 1:numel(dodr_cwt_repo(target_cwts(i)).class)
            k = k + 1;
            if flag_list(k)
...
function forwardButtonPushed(src,event,dodr_cwt_repo, c_flag)
global list_i flag_list ind_list
...
list_i = list_i + 1;
cwt = dodr_cwt_repo(ind_list(list_i, 1)).class(ind_list(list_i, 2)).cue_cwt;
...
function backwardButtonPushed(src,event,dodr_cwt_repo, c_flag)
global list_i flag_list ind_list
if list_i == 1
