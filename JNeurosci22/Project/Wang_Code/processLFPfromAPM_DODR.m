%Convert missing DODR apm files
clear
close all
zw_setpath;
%%  Define task id
...
%%  Step1: Process DODR files by taking timestamps from behavior file
progress_counter = 0;
processed_counter = 0;
processed_files = [];
...
%             end
        ...
        for j = 1:numel(channels)
            ...
            save(fullfile(lfp_database2, [fname, '_CH', num2str(channels(j)), '.mat']), 'LFPData')
        end
...
load('D:\Database\Zhengyang_Wang\Projects\LFP_project_v0.2\Wang_Database\neuron_tbl_w_pfc.mat');
load('D:\Database\Zhengyang_Wang\Projects\LFP_project_v0.2\Wang_Database\compelte_neuron_repo.mat');
%%
lfp_files = dir(fullfile(lfp_database2, file_identifier));
err_log = [];
err_mat = [];
converted_file_id = [];
    ...
    if ~isempty(match_)
        ...
        while run_flag && (neuron_counter < numel(match_))
            ...
            if isempty(neuron_repo(match_(neuron_counter)).class)
                ...
                fid = fopen(fullfile(lfp_database2, 'match_timestamp_err_log_dodr.txt'), 'wb+');
                fwrite(fid, err_log);
                fclose(fid);
                continue
            end
