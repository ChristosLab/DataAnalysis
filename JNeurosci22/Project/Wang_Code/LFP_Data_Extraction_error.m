function LFPData = LFP_Data_Extraction_error(folder_name,filename,CHAN)
%
...
for n = 1:length(trials)
    try
        if (strcmp(AllData.trials(n).Reward, 'No')) && (~isempty(AllData.trials(n).TargetIn)) % Saccade error trials only
