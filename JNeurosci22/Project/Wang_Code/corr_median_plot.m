function corr_median_plot(inputs, f_range, h, labels, color_map, varargin)
yl = [-0.05, 0.3];
...
for i = 1:numel(inputs)
    p(i) = plot(f_range, nanmedian(inputs{i}, 1), 'DisplayName', [labels{i}, sprintf(' (N = %d)', size(inputs{i}, 1))], 'Color', color_map{i}, 'LineWidth', 1.2);
    if min(size(inputs{i})) > 1
        fill(...
            [f_range, fliplr(f_range)], ...
            [prctile(inputs{i}, 25, 1), fliplr(prctile(inputs{i}, 75, 1))], color_map{i}, 'FaceAlpha', 0.15, 'EdgeAlpha', 0.15);
    end
end
for i = 1:size(h, 1)
    h_ = h(i, :);
