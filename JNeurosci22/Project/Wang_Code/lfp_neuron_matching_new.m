...
load(fullfile(project_dir, output_database, 'maunal_session_rejection_flag.mat'));
load(fullfile(project_dir, output_database, 'tfr_delay_tuning_flag.mat'));
load(fullfile(project_dir, output_database, 'neuron_epoch_delay_tuning_flag.mat'));
%%
...
site_tuning_cat_hg = cell(2, 4, 2, 2);
for i_stage = 1:2
    for i_monk = 1:4
        %               stage,   monkey, gamma tuning, spiking tuning
        temp_set = intersect(t{i_stage, i_monk}, find(any(mapping_mat.*~tfr_delay_tuning_flag(:, 3), 2))');
        site_tuning_cat{i_stage, i_monk, 1, 2} = intersect(temp_set, find(any(mapping_mat.*neuron_epoch_delay_tuning_flag, 2))');
        site_tuning_cat{i_stage, i_monk, 1, 1} = setxor(temp_set, [site_tuning_cat{i_stage, i_monk, 1, 2}]);
        temp_set = intersect(t{i_stage, i_monk}, find(any(mapping_mat.*tfr_delay_tuning_flag(:, 3), 2))');
        site_tuning_cat{i_stage, i_monk, 2, 2} = intersect(temp_set, find(any(mapping_mat.*neuron_epoch_delay_tuning_flag, 2))');
        site_tuning_cat{i_stage, i_monk, 2, 1} = setxor(temp_set, [site_tuning_cat{i_stage, i_monk, 2, 2}]);
        temp_set = intersect(t{i_stage, i_monk}, find(any(mapping_mat.*~tfr_delay_tuning_flag(:, 4), 2))');
        site_tuning_cat_hg{i_stage, i_monk, 1, 2} = intersect(temp_set, find(any(mapping_mat.*neuron_epoch_delay_tuning_flag, 2))');
        site_tuning_cat_hg{i_stage, i_monk, 1, 1} = setxor(temp_set, [site_tuning_cat_hg{i_stage, i_monk, 1, 2}]);
        temp_set = intersect(t{i_stage, i_monk}, find(any(mapping_mat.*tfr_delay_tuning_flag(:, 4), 2))');
        site_tuning_cat_hg{i_stage, i_monk, 2, 2} = intersect(temp_set, find(any(mapping_mat.*neuron_epoch_delay_tuning_flag, 2))');
        site_tuning_cat_hg{i_stage, i_monk, 2, 1} = setxor(temp_set, [site_tuning_cat_hg{i_stage, i_monk, 2, 2}]);
    end
end
...
for i_stage = 1:2
    for i_monk = 1:4
        ...
        site_mod_cat{i_stage, i_monk, 3} = intersect(t{i_stage, i_monk}, find(any(mapping_mat.*~tfr_delay_modulation_flag(:, 4, 1), 2))');
        site_mod_cat{i_stage, i_monk, 4} = intersect(t{i_stage, i_monk}, find(any(mapping_mat.*tfr_delay_modulation_flag(:, 4, 1), 2))');        
    end
end
%%
save(fullfile(project_dir, output_database, 'neuron_site_categories.mat'), 'site_mod_cat', 'site_tuning_cat', 'neuron_mod_cat', 'neuron_tuning_cat', 'mapping_mat', 'responsive');
%%
save(fullfile(project_dir, output_database, 'site_tuning_cat_hg.mat'), 'site_tuning_cat_hg');
%%
...
title('High-gamma Power')
xlabel('Time from cue onset (s)')
fig_name = ['high_gamma_power_modulation_by_gamma_power_modulation'];
% print(gcf, fullfile(project_dir, fig_lib, fig_name), '-dpng', '-r400');
