...
%sample from differenct sized pool with same proportion of CS and NMS
...
for i=1:length(find_index) %loop through cells to find cells with more than 5 matcha and nonmatch trials
...
for i=1:length(data_inclusion_index) %loop cells to get spike count
...
%all_winning_models=[];
       ...
       feature_resample=randperm(8);
       if swap_match==1
          reorder_temp_response=temp_response([feature_resample+8,feature_resample],:);
       else
          reorder_temp_response=temp_response([feature_resample,feature_resample+8],:);
       end
      ...
      all_winning_models(p,n)=mean(winning_models);
end
end
mean_winning_model= mean(all_winning_models);
ci_winning_model=1.96*std(all_winning_models)/sqrt(50);
