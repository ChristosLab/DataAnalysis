...
mode=1; %1 for sample period 2 for sampledelay epriod
for i=1:length(final_cells)
    ...
    for j=1:length(final_conditions)
        class_correct=cell_correct{ceil(final_conditions(j)/2)};
        if mod(final_conditions(j),2)==1 %judge if the condition is match or nonmatch
        temp_index=find([class_correct.IsMatch]==1);
        else
        temp_index=find([class_correct.IsMatch]==0);
        end
        ...
        class_err=cell_err{ceil(final_conditions(j)/2)};
        if mod(final_conditions(j),2)==1 %judge if the condition is match or nonmatch
        temp_index=find([class_err.IsMatch]==1);
        else
        temp_index=find([class_err.IsMatch]==0);
        end
        if mode==1 %extract sample or sampledelay period
