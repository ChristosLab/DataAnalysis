...
load spatial_sampledelay_f.mat
spatial_sampledelay_f=f_sampledelay;
...
load spatial_sampledelay_info.mat;
spatial_info=category_sampledelay_info;
...
hold on;
title('spatial cells');
scatter(ones(1,length(spatial_NMS_overlap)),spatial_sampledelay_f(find_index(spatial_NMS_overlap),3));
scatter(2*ones(1,length(spatial_NMS_nonoverlap)),spatial_sampledelay_f(find_index(spatial_NMS_nonoverlap),3));
subplot(1,2,2);
hold on;
title('conj cells');
...
%}
count1=length(intersect(find(spatial_list==1),find(conj_list==1)));
count2=length(intersect(find(spatial_list==1),find(conj_list~=1)));
count3=length(intersect(find(spatial_list~=1),find(conj_list==1)));
count4=length(intersect(find(spatial_list~=1),find(conj_list~=1)));
